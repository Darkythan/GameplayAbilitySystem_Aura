	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // .5 padding for floating point [im]precision
		{
			...
			if (bDebuff)
			{
				...
				UAuraAbilityFunctionLibrary::SetIsSuccessfulDebuff(ContextHandle, true);
				UAuraAbilityFunctionLibrary::SetDamageType(ContextHandle, DamageType);

				const float DebuffDamage = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Damage, false, -1.f);
				const float DebuffDuration = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Duration, false, -1.f);
				const float DebuffFrequency = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Frequency, false, -1.f);

				UAuraAbilityFunctionLibrary::SetDebuffDamage(ContextHandle, DebuffDamage);
				UAuraAbilityFunctionLibrary::SetDebuffDuration(ContextHandle, DebuffDuration);
				UAuraAbilityFunctionLibrary::SetDebuffFrequency(ContextHandle, DebuffFrequency);
			}
		}
	}
}
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair  : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilityFunctionLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			UGameplayStatics::ApplyRadialDamageWithFalloff(
				...
				DamageTypeValue,
				0.f,
				UAuraAbilityFunctionLibrary::GetRadialDamageOrigin(EffectContextHandle),
				UAuraAbilityFunctionLibrary::GetRadialDamageInnerRadius(EffectContextHandle),
				UAuraAbilityFunctionLibrary::GetRadialDamageOuterRadius(EffectContextHandle),
				1.f,
	...
	UAuraAbilityFunctionLibrary::SetIsBlockedHit(EffectContextHandle, bBlocked);
	...
	const UCharacterClassInfo* CharacterClassInfo = UAuraAbilityFunctionLibrary::GetCharacterClassInfo(SourceAvatar);
	...
	UAuraAbilityFunctionLibrary::SetIsCriticalHit(EffectContextHandle, bCriticalHit);
